...
class MasriScheduler:
    ...
    def _task_integration_refresh(self):
        """
        Daily refresh: re-pull Telivy + Entra data for all mapped tenants,
        store in ConfigStore, and optionally re-run LLM auto-process.
        """
        if not self._app:
            return
        ...
        with self._app.app_context():
            try:
                ...
                # Find all tenants with Telivy scan mappings
                mapping_record = ConfigStore.find("telivy_scan_mappings")
                if not mapping_record or not mapping_record.value:
                    return

                all_mappings = json.loads(mapping_record.value)
                # Build {tenant_id: [scan_ids]}
                tenant_scans = {}
                for item_id, mapping in all_mappings.items():
                    if isinstance(mapping, str):
                        ...
                        continue
                    if tid:
                        tenant_scans.setdefault(tid, []).append({
                            "id": item_id,
                            "type": mapping.get("type", "scan") if isinstance(mapping, dict) else "scan",
                        })
                ...
                refreshed = 0
                for tenant_id, scans in tenant_scans.items():
                    try:
                        existing = {}
                        ...
                        if api_key:
                            ...
                            for scan_info in scans[:3]:
                                scan_id = scan_info["id"]
                                scan_type = scan_info.get("type", "scan")
                                try:
                                    if scan_type == "assessment":
                        ...
                        try:
                            ...
                            if entra_cfg and entra_cfg.is_fully_configured():
                                ...
                                try:
                                    ...
                                    entra_data["users"] = {"count": len(users), "sample": users[:5]}
                                except Exception:
                                    pass
                                try:
                                    compliance = entra_client.assess_compliance()
                                    entra_data["compliance"] = compliance
                                except Exception:
                                    pass
                                if entra_data:
                                    existing["entra"] = entra_data
                        except Exception:
                            pass
                        ...
                        ConfigStore.upsert(
                            ...
                            json.dumps(existing, default=str)[:50000],
                        )
                        refreshed += 1
                    except Exception:
