...
| ~~`has_evidence(id=...)` uses `int(id)` on shortuuid string~~ | ~~`app/utils/mixin_models.py`~~ | ~~Medium~~ | **Resolved 2026-04-19** — lookup path now compares shortuuid IDs as strings. Same pass tightened `has_evidence()` so auto-generated stubs (groups `auto_evidence` + `integration_scan` with no file) no longer count as real evidence. |
...
| ~~`pc.status == "not started"` reported on a 99%-complete project~~ | ~~`app/utils/mixin_models.py` (`ControlMixin.generate_stats`)~~ | ~~High~~ | **Resolved 2026-04-19** — root cause was `has_evidence()` treating LLM-authored stub evidence (groups `auto_evidence` + `integration_scan`, no file) as real evidence. A control with 100 % impl + stubs → 100 % completion → project aggregated near 99 %, while `generate_stats` still read subcontrol state honestly. Tightening `has_evidence()` rolls the false completion back to reality. |
