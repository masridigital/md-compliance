...
def authed_client(app_and_db, client):
    """Authenticate as admin by writing Flask-Login session keys directly."""
    app, db = app_and_db
    with app.app_context():
        from app.models import User
        user = db.session.execute(db.select(User).filter_by(email="api_admin@test.com")).scalars().first()
