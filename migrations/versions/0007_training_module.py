...
Revision ID: 0007
Revises: 0006_framework_deprecated
...
revision = "0007"
down_revision = "0006_framework_deprecated"
branch_labels = None
depends_on = None
...
def upgrade():
    if not _table_exists("training"):
        op.create_table(
            "training",
            sa.Column("id", sa.String(8), primary_key=True),
            sa.Column("tenant_id", sa.String(8), sa.ForeignKey("tenants.id"), nullable=False, index=True),
    ...
    if not _table_exists("training_assignment"):
        op.create_table(
            "training_assignment",
            sa.Column("id", sa.String(8), primary_key=True),
            sa.Column("tenant_id", sa.String(8), sa.ForeignKey("tenants.id"), nullable=False, index=True),
