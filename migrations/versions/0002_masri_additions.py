"""Masri Digital additions — settings, branding, storage, LLM, SSO, notifications, due dates, WISP, MCP

Revision ID: 0002_masri_additions
Revises: 0001_initial_schema
Create Date: 2026-03-17

Adds all 11 new tables introduced by the Masri Digital compliance platform.
Safe to run against an existing gapps database — only adds new tables,
never modifies or drops existing ones.
"""

from alembic import op
import sqlalchemy as sa

revision = "0002_masri_additions"
...
def _table_exists(table_name):
    """Check if a table already exists (idempotency guard)."""
...
def upgrade():
    ...
    if not _table_exists("platform_settings"):
        op.create_table(
            ...
            sa.Column("app_name", sa.String(255), server_default="MD Compliance"),
    ...
    # ── TenantBranding ───────────────────────────────────────────────────────
    if not _table_exists("tenant_branding"):
        op.create_table(
            "tenant_branding",
            sa.Column("id", sa.String(), primary_key=True),
    ...
    # ── SettingsSSO ──────────────────────────────────────────────────────────
    if not _table_exists("settings_sso"):
        op.create_table(
            "settings_sso",
            sa.Column("id", sa.String(), primary_key=True),
    ...
    # ── SettingsLLM ──────────────────────────────────────────────────────────
    if not _table_exists("settings_llm"):
        op.create_table(
            "settings_llm",
            sa.Column("id", sa.String(), primary_key=True),
    ...
    # ── SettingsStorage ──────────────────────────────────────────────────────
    if not _table_exists("settings_storage"):
        op.create_table(
            "settings_storage",
            sa.Column("id", sa.String(), primary_key=True),
    ...
    if not _table_exists("settings_notifications"):
        op.create_table(
    ...
    # ── WISPDocument ─────────────────────────────────────────────────────────
    if not _table_exists("wisp_document"):
        op.create_table(
            "wisp_document",
            sa.Column("id", sa.String(), primary_key=True),
            sa.Column("tenant_id", sa.String(), sa.ForeignKey("tenant.id"), nullable=False),
    ...
    # ── WISPVersion ──────────────────────────────────────────────────────────
    if not _table_exists("wisp_version"):
        op.create_table(
            "wisp_version",
            sa.Column("id", sa.String(), primary_key=True),
    ...
    # ── MCPAPIKey ─────────────────────────────────────────────────────────────
    if not _table_exists("mcp_api_key"):
        op.create_table(
            "mcp_api_key",
            sa.Column("id", sa.String(), primary_key=True),
            sa.Column("tenant_id", sa.String(), sa.ForeignKey("tenant.id"), nullable=False),
